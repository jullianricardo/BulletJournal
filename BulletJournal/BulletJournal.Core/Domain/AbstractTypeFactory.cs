...
namespace BulletJournal.Core.Domain
{
    ...
    public static class AbstractTypeFactory<BaseType>
    {
        ...
        /// <summary>
        /// Override already registered type to new type
        /// </summary>
        /// <typeparam name="OldType">Old type</typeparam>
        /// <typeparam name="NewType">New type</typeparam>
        ...
        public static BaseType TryCreateInstance(string typeName)
        {
            ...
            else
            {
                ...
                if (baseType.IsAbstract)
                    throw new OperationCanceledException($"A type with {typeName} name is not registered in the AbstractFactory, " +
                        $"you cannot create an instance of an abstract class {baseType.Name} " +
                        $"because is does not have a complete implementation");
    ...
    public class TypeInfo<BaseType>
    {
        ...
        public IEnumerable<Type> AllSubclasses => Type.GetTypeInheritanceChainTo(typeof(BaseType)).ToArray();
    }
}
